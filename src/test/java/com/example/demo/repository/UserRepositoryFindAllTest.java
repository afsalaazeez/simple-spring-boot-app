...
class UserRepositoryFindAllTest {
	...
	void findAllReturnsNewArrayListInstance() {
		// Arrange - repository initialized
		// Act
	...
	void findAllReturnsRemainingUsersAfterDeletion() {
		...
		Long idToDelete = initialUsers.get(0).getId();
	...
	void findAllReturnsUsersWithCorrectAttributes() {
		// Arrange - repository initialized with sample data
		// Act
		List<User> result = userRepository.findAll();
		// Assert
		assertNotNull(result);
	...
	void findAllReflectsUpdatedUserData() {
		...
		assertNotNull(result);
		assertEquals(3, result.size());
		Optional<User> updatedUser = result.stream().filter(u -> u.getId().equals(userId)).findFirst();
	...
	void findAllReflectsStateAfterDeleteByEmail() {
		...
		assertNotNull(result);
		assertEquals(2, result.size());
		boolean aliceExists = result.stream().anyMatch(user -> "alice@example.com".equals(user.getEmail()));
