...
class UserRepositoryCountTest {
	...
	void countReturnsInitialUserCountAfterConstruction() {
		// Arrange - repository is already created in setUp with 3 initial users
		// Act
	...
	void countDecreasesAfterDeletingSingleUser() {
		// Arrange
		User userToDelete = userRepository.findAll().stream().findFirst().orElse(null);
	...
	void countReturnsCorrectValueAfterMultipleSaves() {
		...
		User user2 = new User("Eve Davis", "eve@example.com", "USER");
		User user3 = new User("Frank Miller", "frank@example.com", "ADMIN");
		// Act
	...
	void countReturnsLongTypeValue() {
		// Arrange - repository is already created in setUp
		// Act
		long count = userRepository.count();
		// Assert
		assertTrue(count >= 0);
	...
	@Test
	@Tag("boundary")
	void countReturnsCorrectValueAfterAddingAndDeletingUsers() {
		// Arrange
		User newUser = new User("David Wilson", "david@example.com", "USER");
		userRepository.save(newUser);
	...
	@Test
	@Tag("invalid")
	void countRemainsUnchangedAfterDeleteByNonExistentId() {
		// Arrange - repository starts with 3 users
		// Act
