...
class ProductRepositoryFindAllTest {
	...
	void findAllReturnsUpdatedProductData() {
		...
		Product productToUpdate = initialProducts.stream()
			.filter(p -> "Laptop".equals(p.getName()))
			.findFirst()
			.orElse(null);
		assertNotNull(productToUpdate);
		Long productId = productToUpdate.getId();
		Product updatedProduct = new Product("Laptop Pro", "Ultra high-performance laptop", new BigDecimal("1499.99"),
				10);
		...
		assertEquals("Laptop Pro", retrievedProduct.getName());
		assertEquals(new BigDecimal("1499.99"), retrievedProduct.getPrice());
	}
	...
	void findAllHandlesConcurrentAccess() throws InterruptedException {
		...
		for (int i = 0; i < threadCount; i++) {
			...
			executorService.submit(() -> {
				try {
					// Some threads read, some threads write
					if (index % 2 == 0) {
						List<Product> products = productRepository.findAll();
						assertNotNull(products);
					}
						...
						productRepository
							.save(new Product("Product" + index, "Description", new BigDecimal("10.00"), 5));
					}
				}
				catch (Exception e) {
		...
		assertNotNull(finalProducts);
		assertTrue(finalProducts.size() >= 5);
	}
	...
	void findAllReturnsConsistentResultsOnMultipleCalls() {
		...
		for (Product product1 : firstCall) {
			boolean found = secondCall.stream()
				.anyMatch(product2 -> product1.getId() != null && product1.getId().equals(product2.getId()));
			assertTrue(found, "Product with ID " + product1.getId() + " should be present in both calls");
		}
	}

	@Test
	@Tag("valid")
	void findAllReturnsProductsWithValidFields() {
		// Arrange - repository already initialized
		// Act
		List<Product> products = productRepository.findAll();
		// Assert
		...
		for (Product product : products) {
			...
			// Note: Removed getQuantity() check as Product class does not have this
			// method
			// If quantity field is needed, consider adding getQuantity() method to
			// Product class
		}
	}
	...
	@Test
	@Tag("valid")
	void findAllReturnsDistinctProducts() {
		// Arrange - repository already initialized
		// Act
		List<Product> products = productRepository.findAll();
		// Assert
		assertNotNull(products);
		long distinctCount = products.stream().map(Product::getId).distinct().count();
		assertEquals(products.size(), distinctCount, "All products should have unique IDs");
	}
