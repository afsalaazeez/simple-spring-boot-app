...
class ProductRepositoryFindAllTest {
	...
	void findAllIncludesNewlySavedProduct() {
		...
		boolean containsTablet = products.stream().anyMatch(p -> "Tablet".equals(p.getName()));
		assertTrue(containsTablet);
	}
	...
	void findAllExcludesDeletedProduct() {
		...
		boolean containsDeletedProduct = products.stream().anyMatch(p -> p.getId() != null && p.getId().equals(1L));
		assertFalse(containsDeletedProduct);
	}
			...
			executorService.submit(() -> {
				try {
					...
					if (index % 2 == 0) {
						...
						Product newProduct = new Product("Product" + index, "Description", new BigDecimal("10.00"), 5);
						productRepository.save(newProduct);
					}
				}
				catch (Exception e) {
