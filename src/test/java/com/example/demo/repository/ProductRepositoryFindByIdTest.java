...
class ProductRepositoryFindByIdTest {
	...
	void findByIdWithExistingProductReturnsProduct() {
		// Arrange - repository is pre-populated with sample data during construction
		Long existingId = 1L;
		// Act
	...
	void findByIdAfterDeletionReturnsEmptyOptional() {
		// Arrange
		Long idToDelete = 1L;
		Optional<Product> beforeDeletion = productRepository.findById(idToDelete);
		assertTrue(beforeDeletion.isPresent(), "Product should exist before deletion");
	...
	@Test
	@Tag("valid")
	void findByIdReturnsSameProductInstanceFromMap() {
		// Arrange
		Long existingId = 2L;
		// Act
		Optional<Product> firstCall = productRepository.findById(existingId);
		Optional<Product> secondCall = productRepository.findById(existingId);
		// Assert
